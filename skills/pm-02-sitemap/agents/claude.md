...
Follow `SKILL.md` for lifecycle and merge rules. Use `assets/sitemap-template.md` as the fixed Markdown schema. Keep headings, headerless status HTML table, layout mind map/table, sitemap mind map/table, assumption-list labels, table columns, and ID formats exactly consistent.

## Mandatory Blocking Check For Claude
...
Determine the required application end/form documents from Product Overview section `2.产品设计概览`, especially `2.3.产品端与形态表`.
...
When updating an existing sitemap, treat direct user edits in sections `1` and `2` as approved content unless superseded by newer instructions, merge user replies from `3.待确认与假设` and `4.用户补充说明`, and preserve stable layout IDs and `PAGE-xxx` IDs unless newer source evidence or user review input supersedes them.
