...
此 skill 用于把 pm06 生成的多页面 Figma JSON 节点文档集中写入 Figma。它只负责编排、筛选、任务追踪和失败续跑；单个页面的 Figma 还原规则必须由 `pm-08-md2figma` 执行，包括 RenderPlan、TokenResolver、组件蓝图、renderSpec、两阶段布局和 QA。
