...
`pm-09-md2figma-batch` 是纯批量编排 skill。它不会改变 `pm-08-md2figma` 的执行方式，也不会自己实现单页 Figma 还原。

对每个通过预检查的 `figma-nodes.json`，正常写入模式下的批量循环必须是：

```text
for each valid figma-nodes.json in task-list order:
  mark the task In Progress
  invoke pm-08-md2figma once as a fresh single-page execution for that JSON only
  validate the pm08 restore report and post-render QA result
  mark the task Done or Failed
```

硬性要求：

- 每次循环必须只针对一个 `figma-nodes.json` 调用一次 `pm-08-md2figma`。
- 每次 `pm-08-md2figma` 调用必须是新的单页执行。不得把上一个页面的对话上下文、生成推理或临时指令带入下一个页面。跨页面共享只能通过文件系统标准产物发生，例如任务清单、源 JSON、同目录 notes/report、profile 文件和 Figma 文件中的已写入结果。
- 不得把多个 JSON、多个页面、多个 profile group、多个目录或多个产品端合并为一次下游调用。
- 不得因为用户输入、上下文生成的新指令、性能优化、模型限制或便利性而发明新的批量写入方式。
- 不得在 pm09 内联、摘要、改写、套用或重新实现 `pm-08-md2figma` 的单页还原规则。pm09 只能筛选任务、维护任务状态、逐项调用 pm08，并校验 pm08 产物。
- 如果运行环境无法把某个任务作为独立 skill 调用 `pm-08-md2figma`，必须将该任务标记为 `Failed`，原因写为 `pm08 invocation unavailable`；不得退化为 pm09 自己写入 Figma。
- `dry run` 是唯一不调用 pm08 的例外；该模式只允许扫描、预检查和生成任务清单。
...
pm09 不得绕过 pm08 直接自由写入 Figma。若因 Agent 环境限制无法真正“调用 skill”，必须将受影响任务标记为 `Failed`，原因写为 `pm08 invocation unavailable`；不得逐项套用、复刻或内联执行 pm08 的 `SKILL.md` 规则。
