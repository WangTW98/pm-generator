# PAGE-003 - 微信扫扫登录 Figma Restore Report

## 基本信息

- 源 JSON：`product/pages/客户端-PC-Web/PAGE-001-登录注册/PAGE-003-微信扫扫登录/figma-nodes.json`
...
| 项目 | 结果 | 说明 |
| --- | --- | --- |
| 使用 figmaDocument.children 作为主来源 | pass | 保留 auth 页面结构，未按名称重套模板 |
| RenderPlan 节点总数 | pass | Figma 结果 89 节点 |
| 保留原始节点顺序 | pass | Topbar、Footer、PageBody、AuthPanel 顺序可追踪 |
| 保留 sourceNodeId metadata | pass | 写入 `pm08_md2figma/sourceNodeId` |
| 合并 renderSpec.componentInstances | pass | Tabs、QR credential slot、Button、Checkbox、Text fallback 已生成 |
...
| pageIntent 仅用于校验 | pass | 未改写为列表/表格页面 |
...
| `PageBody/PAGE-003-微信扫扫登录` | Frame | pass | 页面本体未混入登录后侧栏 |
| `Section/AuthPanel` | Frame | pass | 认证卡片独立区域 |
| 登录方式切换 | Frame/Text | pass | 微信扫码为 active item |
| 二维码凭证槽 | Frame/Text/Rectangle | pass | 二维码为可编辑矩形网格，不是截图 |
| 刷新二维码 | Frame/Text | pass | 作为辅助动作生成 |
...
| QR Credential Slot | 微信二维码 | fallback | 可编辑 Frame/Rectangle/Text |
| Button | 刷新二维码 | fallback | 可编辑 Frame/Text |
| Checkbox | 协议勾选框 | fallback | Rectangle + Text |
| Tag | 客户账号访问、微信扫码、安全登录 | fallback | 可编辑状态标签 |
...
| qaExpectations | 6 | 0 | 关键 auth/QR 区命中 |
...
| 文本未明显挤压或溢出 | pass | QR、按钮、协议文本均在卡片内 |
| 组件最小尺寸与文本策略已应用 | pass | 刷新二维码按钮 228 x 40 |
| Profile locked 尺寸/间距已应用 | pass | AuthPanel、Topbar、Footer、body 按 profile 稳定尺寸 |
| 主要容器无明显堆叠 | pass | 截图与结构 QA 均通过 |
| 空白位置追加且未覆盖原内容 | pass | 新根画板为 `(2)`，旧 `PAGE-003 - 微信扫扫登录 - PC Web` 保留 |
...
| 重复可见文本 | pass | 导航和辅助链接属于不同角色 |
| 重复 actionIntent | pass | 无重复 primary 登录按钮 |
| 重复 primary action | pass | 刷新二维码是辅助动作 |
| 允许的角色差异 | pass | 微信扫码 active tab 与二维码区共享同一 credential slot |
...
| 必备区域命中 | pass | AuthPanel、AuthMethodSwitch、QR credential slot、refresh action、agreement、auxiliary links |
| 源 JSON 是否包含关键区 | pass | renderSpec.qaExpectations 已声明 auth 关键区 |
...
| 项目 | 结果 | 说明 |
| --- | --- | --- |
| 截图 URL | pass | `https://www.figma.com/api/mcp/asset/0258fa52-4fd3-4399-bd20-d85a7ba122c1` |
| 直接子节点 | pass | `GlobalShell/Topbar`、`GlobalShell/Footer`、`PageBody/PAGE-003-微信扫扫登录` |
| 总节点数 | pass | 89 |
| Frame 节点数 | pass | 16 |
| Text 节点数 | pass | 22 |
| Rectangle 节点数 | pass | 51 |
| 关键文本样本 | pass | 合规服务平台、Case Tracking Portal、微信扫码登录、等待扫码、刷新二维码 |
...
- 源 layout 的元素表仍使用通用账号输入/验证码命名；本次按 `figma-nodes.json.semanticValidation.autoFixes` 还原为微信扫码凭证槽。
- 基础组件未导入真实设计系统组件，使用可编辑 fallback 还原。

## 人工检查项

- 对比 `PAGE-003 - 微信扫扫登录 - PC Web` 与新建的 `PAGE-003 - 微信扫扫登录 - PC Web (2)` 的视觉差异。
