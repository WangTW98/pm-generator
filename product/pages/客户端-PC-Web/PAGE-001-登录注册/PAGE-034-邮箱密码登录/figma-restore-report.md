# PAGE-034 - 邮箱密码登录 Figma Restore Report
...
| RenderPlan 节点总数 | pass | Figma 结果 43 节点 |
...
| `PageBody/PAGE-034-邮箱密码登录` | Frame | pass | 页面本体未混入登录后侧栏 |
| `Section/AuthPanel` | Frame | pass | 认证卡片独立区域 |
| 登录方式切换 | Frame/Text | pass | 邮箱密码为 active item |
| 邮箱输入 | Frame/Text | pass | 邮箱输入框可编辑 |
| 密码输入 | Frame/Text | pass | 密码占位使用掩码展示 |
| 协议与帮助 | Frame/Text/Rectangle | pass | 协议勾选框、忘记密码与切换入口 |
...
| Input | 邮箱、密码 | fallback | 可编辑输入框结构 |
| Button | 登录 | fallback | 可编辑 Frame/Text |
| Checkbox | 协议勾选框 | fallback | Rectangle + Text |
| Tag | 客户账号访问、邮箱密码、安全认证 | fallback | 可编辑状态标签 |
...
| 文本未明显挤压或溢出 | pass | 邮箱、密码、登录按钮均在卡片内 |
| 组件最小尺寸与文本策略已应用 | pass | 输入与按钮高度 44 |
| Profile locked 尺寸/间距已应用 | pass | AuthPanel、Topbar、Footer、body 按 profile 稳定尺寸 |
| 主要容器无明显堆叠 | pass | 截图与结构 QA 均通过 |
| 空白位置追加且未覆盖原内容 | pass | 新根画板为 `(2)`，旧 `PAGE-034 - 邮箱密码登录 - PC Web` 保留 |
...
| 必备区域命中 | pass | AuthPanel、AuthMethodSwitch、email input、password input、agreement、primary submit、auxiliary links |
...
| 项目 | 结果 | 说明 |
| --- | --- | --- |
| 截图 URL | pass | `https://www.figma.com/api/mcp/asset/da191c54-6176-4183-8a52-c5c55fd2c350` |
| 直接子节点 | pass | `GlobalShell/Topbar`、`GlobalShell/Footer`、`PageBody/PAGE-034-邮箱密码登录` |
| 总节点数 | pass | 43 |
| Frame 节点数 | pass | 17 |
| Text 节点数 | pass | 25 |
| Rectangle 节点数 | pass | 1 |
| 关键文本样本 | pass | 合规服务平台、Case Tracking Portal、邮箱密码登录、请输入邮箱地址、登录 |
...
- 对比 `PAGE-034 - 邮箱密码登录 - PC Web` 与新建的 `PAGE-034 - 邮箱密码登录 - PC Web (2)` 的视觉差异。
