# 待办事项 Figma Node Notes

## 0.生成状态

| 项目 | 内容 |
|---|---|
| 源页面 | `product/pages/客户端-PC-Web/PAGE-007-工作台/PAGE-008-待办事项/layout.md` |
| 来源Sitemap | `product/layout/客户端-PC-Web-sitemap.md` |
| 使用Layout | 客户端 / PC Web |
| 页面清单ID | PAGE-008 |
| 状态组 | 无 |
| 使用设计规范 | `design/antd-admin-design-spec` |
| 输出JSON | `product/pages/客户端-PC-Web/PAGE-007-工作台/PAGE-008-待办事项/figma-nodes.json` |
| 生成日期 | 2026-05-19 |
...
- 页面挂载容器：`PageBody/PAGE-008-待办事项`，位于 `Frame/Primary/客户端 / PC Web` 内。
...
| 页面意图 | dashboard |
| 置信度 | high |
| 主要证据 | 页面属于工作台任务处理场景；默认元素包含指标概览、优先级筛选、待办卡片和处理入口 |
| 使用语义模式 | `page-intent-patterns.patterns.dashboard` |
| 语义校验状态 | pass |
| 冲突与修复 | 无 |
...
|---|---|---|---|---|---|
| 待办概览区 | 今日待办数 | 文本/展示 | TEXT | Text | 默认状态元素，映射到可编辑节点 |
| 待办概览区 | 即将超时数 | 文本/展示 | TEXT | Text | 默认状态元素，映射到可编辑节点 |
| 优先级筛选区 | 优先级筛选 | 选择控件 | COMPONENT_INSTANCE | Select | 默认状态元素，映射到可编辑节点 |
| 优先级筛选区 | 待办事项卡片 | 文本/展示 | COMPONENT_INSTANCE | Card | 默认状态元素，映射到可编辑节点 |
| 待办列表区 | 关联案件 | 文本/展示 | TEXT | Text | 默认状态元素，映射到可编辑节点 |
| 待办列表区 | 处理按钮 | 按钮 | COMPONENT_INSTANCE | Button | 默认状态元素，映射到可编辑节点 |
| 处理入口区 | 忽略/稍后提醒按钮 | 按钮 | COMPONENT_INSTANCE | Button | 默认状态元素，映射到可编辑节点 |
...
| 关键区域QA | keyRegions=PageHeader, MetricGrid, PriorityFilter, TaskCardList, ActionBar；forbiddenRegions=duplicated global shell, blocking duplicate primary action |
| 还原风险 | 无 |
...
| pageIntentKey | dashboard |
| stateGroupKey | 无 |
| Layout Profile | `product/style-profiles/客户端-PC-Web/layout-profile.json` |
| Intent Profile | `product/style-profiles/客户端-PC-Web/intent-dashboard-profile.json` |
| State Group Profile | `无` |
| Profile状态 | layout=created, intent=created, state=not-applicable |
| layoutHash | b2968cd4bc2c |
| intentHash | b63cfa7144ff |
| stateGroupHash | 无 |
| 锁定属性 | canvas；global shell；content padding；responsive fallback；dashboard skeleton；no state group |
